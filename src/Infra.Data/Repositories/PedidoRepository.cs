...
namespace Infra.Data.Repositories
{
    public class PedidoRepository : IPedidoRepository
    {
        ...
        public async Task<List<Pedido>> ListarPedidos() => await _context.Pedido.Include(x=> x.Produtos).ThenInclude(x => x.Produto).ToListAsync();
        public async Task<Pedido> ObterPorId(long id) => await _context.Pedido.Include(x => x.Produtos).ThenInclude(x => x.Produto).FirstOrDefaultAsync(x => x.Id == id);
